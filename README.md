...
android/    Kotlin + Jetpack Compose native app, consumes the same API
```

The Android app (see `android/README.md`) covers the core customer flow —
browse/search restaurants, menu, cart, auth, order tracking — but hasn't been
built or run, since this machine has no Android SDK. A native **iOS
(Swift)** app, plus a deeper AI/ML/DL layer, are still future phases.
...
- [x] Android app (Kotlin, Jetpack Compose) consuming the same API —
      core customer flow scaffolded, not yet build-verified (`android/`)
- [ ] Staff/kitchen screens on Android
